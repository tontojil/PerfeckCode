---
name: stitch-generar-diseno
description: "Generación de pantallas nuevas desde texto o imágenes con Stitch, edición de pantallas existentes y variantes con tokens de diseño. (generate design, screens, Stitch)"
allowed-tools:
  - "stitch*:*"
  - "Bash"
  - "Read"
...
- **Design System**: Check if a design system exists for the project via
  ...
  are already applied at the project level — do NOT include any color, font, or
   theme instructions in the generation prompt. If none exists, usa stitch-sistema-diseno first before generating screens.
...
handled at the project level via usa stitch-sistema-diseno).
...
> project level via usa stitch-sistema-diseno and will conflict if
...
Upload the image inline: create the screen in the target project, attach the
image as screen content/reference, then continue to step 3 (no external skill needed).
...
file tracks all screens, their device types, and design system info. For the format,
usa stitch-sistema-diseno (ver su ejemplo de metadata.json).
